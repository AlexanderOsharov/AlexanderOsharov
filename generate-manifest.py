        ...
        for ext in image_extensions:
            for file_path in images_dir.glob(ext):
                ...
                if not filename.startswith('avatar'):
                    manifest["images"].append(filename)
    ...
    if videos_dir.exists():
        ...
        for ext in video_extensions:
            for file_path in videos_dir.glob(ext):
                manifest["videos"].append(file_path.name)
    ...
    if audio_dir.exists():
        ...
        for ext in audio_extensions:
            for file_path in audio_dir.glob(ext):
                manifest["audio"].append(file_path.name)
...
def main():
    ...
    print("1. Создать manifest из папок (рекомендуется)")
    print("2. Создать упрощенный manifest для тестирования")
    ...
    if choice == "1":
        ...
        print("Неверный выбор. Создаем упрощенный вариант...")
        create_simple_manifest()
